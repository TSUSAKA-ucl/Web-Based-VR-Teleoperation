...
1. Install and Creation of the cert
   * `sudo apt install can-utils mosquitto-dev mkcert python3-venv`
   * `cd && python3 -m venv venv`
     `. ~/venv/bin/activate` and install
     `paho-mqtt` `sora-sdk`
     `coppeliasim_zmqremoteapi_client`
     `piper-sdk`
   * create a server certificate using your own rootCA
     ```
     cd YourPnpmPackageRoot
     mkcert `hostname`.YourDomain 192.168.XX.XX 192.168.YY.YY localhost 127.0.0.1
     [[ ! -e certificates ]] && cd certificates
     ln -s ../YourCert.pem localhost.pem
     ln -s ../YourCertKey.pem localhost-key.pem
     ```
     distribute and configure the rootCA to your necessary clients. for Chromium,
     ```
     certutil -d sql:$HOME/.pki/nssdb -A -t "CT,c,c" -n "My Local Test CA" -i YOUR-OWN-rootCA.pem
     ```
     openssl can use `~/.local/share/mkcert/rootCA.pem` directly
...
2. Run
   ...
   * fetch the VR page from HTTPS server
     this JS connects wss server and mosquitto writes messages with USER_UUID to stdout
   * start AgileX's `piper_ui.py`
     clone from `https://github.com/TSUSAKA-ucl/Piper_sdk_ui.git` branch `ucl-nuee`,
     then if your shell is configured to allow sudo, you do not need to enter a password.
     ```
     python3 piper_ui.py
     ```
     **Find CAN Port** -- **Select can** -- 
     **Reset** --  **Enable** --  **Go Zero**

      
   * confirm and copy `USER_UUDI`, then export the USER_UUID environment variable
     and run robot MQTT bridge `
     ```
     export USER_UUID=xxxx
     ```
     `xxxx` is a string like `82156cf0-e682-49df-9933-2542f33584a4-local`
     ```
